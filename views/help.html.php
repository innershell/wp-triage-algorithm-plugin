...
<ol>
    ...
    <li>Checkbox answers specified with "Next Question" will use go to the next question on the list in
        ...
        checkbox answer, the topic will show this.</li>
    <li>Using <code>None</code> as a checkbox answer is a special reserved word that will automatically
        trigger special handling. When this checkbox is selected, all previous selections will be unchecked.</li>
</ol>
...
<h2>Publishing Topics</h2>
<p>To publish a topic, place its shortcode in a page. You can give the page a patient-friendly title
    so that it is user-friendly.</p>
...
<p>Make sure to publish the page so that the questionnaires are visible website.</p>

<h1>Import/Export Setup Data</h1>
<h2>Export SQL</h2>
<p>Generate a SQL insert script for the Orchestra setup data and the Wordpress
    pages data. Use this to import the data into another Orchestra installation.</p>
<p>If your destination database uses a different prefix, you can add <code>'&prefix='</code> to
    in the URL to specify the prefix used by your destination database. Instead of clicking
    on the link, copy the link's URL, paste it into another browser window. Then append the
    prefix code to the end of the URL.</p>
...
<p><strong>“ALERT: PROVIDER_NOTE not setup for the following question/answer”</strong><br />
    Ensure that the ‘Provider Note’ field has some text entered. Otherwise, it will display the ALERT and append the
    Question/Answer into the SOAP note.</p>

<p><strong>The ‘Provider Note’ text is not used for Text answers.</strong><br />
    Ensure that ‘Answer’ field has some text. The topic uses the text in the ‘Answer’ field as the field label for
    the Topic when presented to the patient.</p>