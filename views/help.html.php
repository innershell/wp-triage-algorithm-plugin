...
<p>A user account is required to perform an algorithm so that Orchestra can 
track which user performed the algorithm. The user account will also be used 
...
Orchestra can administer the <i>Algorithm</i> to the patient and send the final  
...
<ol>
	...
	<li>Set the <i>Role</i> to "Subscriber" to give users access to perform Algorithms.</li>
</ol>
...
<h2>Algorithms</h2>
<p><i>Algorithms</i> are also known as either "<i>Instruments</i>" or "<i>Topics</i>" to be 
administered to a patient. Each <i>Algorithm</i> contains a collection of <i>Questions</i> 
for the patient to answer and corresponding <i>Results</i> that that are determined based on 
the patient's response. Algorithms, Questions, and Results must all be configured by the 
Orchestra administrator.</p>
<ol>
	<li>Login to Orchestra.</li>
	<li>Click on <i>Triage Algorithms</i> on the sidebar.</li>
	<li>Click on the <i>Settings</i> submenu.</li>
	<li>Create a new Algorithm, Questions, and Results. A <i>Shortcode</i> will automatically be generated. Remember this shortcode.</li>
...
<ol>
	<li>Create Questions under the selected Algorithm.</li>
...
<ol>
	...
	checkbox answer, the algorithm will show this.
</ol>
...
<ol>
	<li>Create <i>Results</i> under the selected <i>Algorithm</i>.</li>
	<li>Enter a <i>Min. Points</i> and <i>Max. Points</i> for the result.</li>
	<li>Multiple results can be provided for each <i>Algorithm</i>. However, ensure that the points do not overlap or the algorithm will get "confused".</li>
</ol>

<h2>Shortcodes</h2>
<p>The following shortcodes are currently offered by WindWake Orchestra:</p>
<ul>
	<li>[triage-algorithm XX] - Where XX refers to the algorithm ID.</li>
	<li>[triage-submissions algorithm="XX"] - Where XX refers to the algorithm ID.</li>
</ul>
<br />
...
<ol>
	...
	<li>A <strong>unique WindWake Orchestra login account</strong> is provided to each client to access the <i>Algorithm</i>.</li>
...
Ensure that ‘Answer’ field has some text. The algorithm uses the text in the ‘Answer’ field as the field label for the Algorithm when presented to the patient.</p>