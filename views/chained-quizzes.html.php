<div class="wrap">
	...
	<table class="widefat">
		<tr><th><?php _e('Topic Name', 'chained')?></th><th><?php _e('Shortcode', 'chained')?></th><th><?php _e('Questions', 'chained')?></th>
		...
		<?php foreach($quizzes as $quiz):
			...
			<tr class="<?php echo $class?>"><td><?php if(!empty($quiz->post)) echo "<a href='".get_permalink($quiz->post->ID)."' target='_blank'>"; 
				...
				if(!empty($quiz->post)) echo "</a>";?></td><td><input type="text" size="18" value="[triage-topic <?php echo $quiz->id?>]" readonly onclick="this.select();"></td>
			...
			| <a href="admin.php?page=chained_quizzes&copy=1&id=<?php echo $quiz->id?>" onclick="return confirm('<?php _e('This will make a duplicate of the whole Topic.', 'chained')?>');"><?php _e('Copy', 'chained')?></a>
			| <a href="#" onclick="confirmDelQuiz(<?php echo $quiz->id?>);return false;"><?php _e('Delete', 'chained')?></a></td></tr>
		<?php endforeach;?>	
	</table>
	<p><?php _e('NOTE: If the Topic Name is not hyperlinked, it means the Topic shortcode is not published. To publish, insert the shortcode in a public Post or Page.', 'chained')?></p>
